...
namespace SpecFlow.RestEase.StepDefinitions
{
    ...
    public class UserSteps
    {
        ...
        [Given(@"Username ""(.*)"" and Repository ""(.*)""")]
        public void GivenUsernameAndRepository(string username, string reponame)
        {
            // setting property on interface for later request instead of as method argument
            _usersApi.RepoName = reponame;
            _usersApi.Username = username;
        }
        ...
        public void WhenIGetStarGazersForRepository()
        {
            // make request
            _usersResponse = _usersApi.GetStarGazers().Result;

            // deserialize response to type T
            _users = _usersResponse.GetContent();
        }
        ...
        public void ThenCountOfAllStarredUsersAreDisplayed()
        {
            _users.Should().OnlyHaveUniqueItems(user => user.Id)
                .And.NotContainNulls(user => user.HtmlUrl)
                .And.NotContainNulls(user => user.Url)
                .And.NotContainNulls(user => user.ReposUrl);
            _users.Should().HaveCountGreaterOrEqualTo(0);
        }
    }
}